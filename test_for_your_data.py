...
from utils import YourImageData
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', default=1, type=int, help='batch size on train iter')
    parser.add_argument('--test_path', type=str, default='datasets/your_data', help='npz folder for valid')
    args = parser.parse_args()
    device = torch.device("cuda" if args.gpu_num > 0 else "cpu")

    init_time = time.time()
    test_sets = YourImageData(args.test_path, input='input', label='gt')
    ...
    # create save folder
    save_folder = os.path.join(args.output_root, 'your_test_out')
    ...
    with torch.no_grad():
        ...
        for ti, (input, target, names) in enumerate(test_loader):
            ...
            if args.save_image:
                for fi in range(len(names)):
                    vutils.save_image(fakeB[fi], os.path.join(save_folder, '%s_outputs.png' % names[fi]),
                                      normalize=True)
        ...
        print('test consume time : %f' % (time.time() - init_time))
